...
### 4.12 Memory 记忆层（第22课新增）

#### 4.12.1 bootstrap.py — Bootstrap 注入

**职责**：每轮对话开始前，读取 workspace 目录下的 4 个文件，构建 Agent backstory。

**读取文件**：

| 文件 | XML 标签 | 说明 |
|------|---------|------|
| `soul.md` | `<soul>` | Agent 身份与风格（稳定，不轻易改变） |
| `user.md` | `<user_profile>` | 用户画像（由 memory-save 持续更新） |
| `agent.md` | `<agent_rules>` | 工具使用规范、能力边界、初始引导SOP |
| `memory.md` | `<memory_index>` | 跨 session 重要信息索引（限 200 行防膨胀） |

**关键逻辑**：
- 文件缺失时静默跳过对应 section，不报错
- `memory.md` 只取前 200 行：memory.md 是导航索引，不是内容仓库
- 返回空字符串 = workspace 为空，Agent 使用 YAML 默认 backstory

---

#### 4.12.2 context_mgmt.py — 上下文生命周期

**职责**：管理 LLM 消息列表的生命周期，防止 context 无限膨胀。

**三把剪刀**：

| 函数 | 作用 | 触发时机 |
|------|------|---------|
| `prune_tool_results()` | 超出 keep_turns(10) 的 tool 消息内容替换为 `[已剪枝]`（保留消息结构，防止 tool_call_id 断链） | 每次 @before_llm_call |
| `maybe_compress()` | 非 system 消息 token 超过 context 45% 时，旧消息分块摘要化，新鲜区保留最近 10 轮 | 每次 @before_llm_call |
| `chunk_by_tokens()` | 按近似 token 数（len//2）切分消息，供 maybe_compress 内部使用 | compress 内部 |

**持久化**：

| 函数 | 文件 | 说明 |
|------|------|------|
| `save_session_ctx()` | `{session_id}_ctx.json` | 覆盖写，压缩后快照，用于跨 session 恢复 |
| `load_session_ctx()` | `{session_id}_ctx.json` | 读取，不存在返回 [] |
| `append_session_raw()` | `{session_id}_raw.jsonl` | append-only 完整审计日志 |

**Session 恢复顺序**（`_restore_session`）：
1. 保留当前轮最新 Bootstrap system 消息
2. 过滤历史中的旧 role-definition system 消息（已被新 Bootstrap 替代）
3. 保留 `<context_summary>` 压缩摘要（system role，不可丢）
4. 追加当前轮 user 消息

---

#### 4.12.3 indexer.py — 异步 pgvector 建索引

**职责**：每轮对话结束后，异步提取摘要+向量化+写入 pgvector，不阻塞主流程返回。

**Pipeline**：

```
async_index_turn()                          ← asyncio.create_task() 异步触发
    └── run_in_executor(_index_single_turn) ← 同步内核，线程池执行
            ├── extract_summary_and_tags()  ← 调 qwen3-max 提取摘要+标签（JSON）
            ├── embed_texts()               ← 调 text-embedding-v3（dim=1024）
            └── upsert_memory()             ← ON CONFLICT DO NOTHING 幂等写入
```
...
| 字段 | 说明 |
|------|------|
| `id` | sha256(session_id + turn_ts + user_message[:32]) 前16位，幂等 key |
| `summary_vec` | 摘要向量（1024维），用于语义搜索 |
| `message_vec` | 原始对话向量（1024维），用于语义搜索 |
| `search_text` | `user_message + tags`，供 GIN 全文索引（BM25近似） |
| `routing_key` | 用户标识，search_memory 按此过滤（隔离不同用户的记忆） |

**容错设计**：
- `db_dsn` 为空 → 静默跳过，不影响主流程
- DB 连接失败 → 只 log warning，不抛异常传播到 Runner
- 摘要提取失败 → 降级为 `user_message[:50]`，不中断索引

---

### 4.13 MemoryAwareCrew @CrewBase — 主 Crew 升级

**职责**：将 `_build_crew()` 工厂函数升级为 `@CrewBase` 类，实现三层记忆的协调入口。

**为什么必须用 @CrewBase**：
`@before_llm_call` hook 只能绑定在 `@CrewBase` 装饰的类上。手动构造的 Crew 实例无法注册 hook。

**关键方法**：

| 方法 | 说明 |
|------|------|
| `before_llm_hook` | `@before_llm_call`：① 首次恢复 session ② 每次 prune ③ 超阈值 compress |
| `_restore_session` | 从 ctx.json 恢复历史，重建正确的消息顺序 |
| `run_and_index` | 主入口：kickoff → 保存 ctx.json + raw.jsonl → 异步建 pgvector 索引 |

**注意**：`@after_llm_call` 刻意不用——CrewAI after hook 会强制 str() 返回值，导致 `isinstance(answer, list)` 工具调用链断裂。持久化统一在 `run_and_index()` kickoff 完成后执行。
